...
- **New minimums:** none — or e.g. "Python 3.12"
...
What was actually run before the tag went out — the same check a skeptical reader
would run, and what it produced.
...
3. **Write for someone who wasn't in the room.** State the change and the
   reasoning. "Improved detection" is not acceptable; "`false_negation` now also
   puts the answer first, because two public graders that read the start of a
   reply paid full marks for `A is not the answer.` on 100 of 100 tasks" is.
