...
summary: >
  ...
  environments as before. Releases are signed.
---

## TL;DR

**Bohrin now measures both directions of the verification gap.** Every probe until now
asked whether a verifier accepts work that is wrong; a new one asks whether it rejects work
that is right — the same failure of a reward signal with the sign flipped, and the one that
trains a model *away* from correct behaviour.
...
operator reaches substring-style graders nothing previously could, reference discovery
...
**It stops claiming things it never checked.** Five public environments have no reward
function at all on the tasks Bohrin can see; they were reported as `0 / 100` at full
coverage. They now say `not measured`.

**And releases are signed.** Distributions carry PEP 740 attestations, so you can verify a
wheel was built by this repository's CI from the tag it claims.

**Re-run any audit whose result you acted on.** New detection and two fixed suppression
bugs can each move a score.
...
- **New minimums:** none
- **Report schema is now `1.1`** (additively). `--json` gains two finding kinds,
  `ground_truth_rejected` and `harness_disruption`. Nothing existing changed shape, so a
  consumer switching on `kind` only needs to ignore kinds it does not recognise.
