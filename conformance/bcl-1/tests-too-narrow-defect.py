"""BCL-1 fixture: tests-too-narrow, with defect BGW-120.
...
The defect: besides checking values, it requires the name the reference solution happens to use
for its running total, so a correct program with a different variable name fails.
"""
...
def grade(submission: str) -> float:
    ...
    if "total" not in submission:  # the defect: an incidental choice of the reference, required
        return 0.0
    try:
