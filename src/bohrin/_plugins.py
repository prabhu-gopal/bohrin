...
The standard-library ``importlib.metadata`` route (non-provisional since 3.10). Built-in and
third-party mutation operators and relations advertise themselves through the same
entry-point groups — there is no privileged path, and nothing here checks a licence.

A plugin whose ``.load()`` raises is skipped with a warning rather than crashing discovery.
One bad plugin must never take down the rest.
"""
