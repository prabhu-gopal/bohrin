...
the malformed-number problem the Verification Gap already forbids for checks, one level
down — and it is not hypothetical: a synthetic environment in which ``weak_oracle`` could
measure only 2 of 20 tasks produced ``VERIFICATION GAP: 50 / 100`` at full coverage, with
nothing beside it to say what it rested on.
...
def rate(affected: int, measured: int) -> dict[str, object]:
    """A proportion in the shape every check reports it: the counts, and the interval."""
